...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (demo) {
      ...
      const userObj = { ...user, id: Math.floor(Math.random() * 1000), token: 'demo-token-' + Date.now() };
      login(userObj, 'demo-token');
      
      // Force-write to localStorage as backup to ensure state persists across page reload
      const authState = { state: { user: userObj, token: 'demo-token', isAuthenticated: true }, version: 0 };
