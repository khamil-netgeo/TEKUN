...
class DisbursementController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $level    = Disbursement::determineAuthority($amount);
        $label    = Disbursement::getAuthorityLabel($level);
        ...
        $disbursement = Disbursement::create(array_merge($validated, [
            ...
            'authority_level_required' => $level,
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // AGING REPORT — GET /api/disbursements/aging-report  (alias: /aging)
    // ─────────────────────────────────────────────────────────────────────────
    public function agingReport(Request $request)
    {
        $disbursements = Disbursement::with(['application:id,applicant_name,scheme'])
            ...
            ->get()
            ->map(function ($d) {
                // Use actual aging_days column if set, else compute from created_at
                $agingDays = ($d->aging_days > 0) ? (int) $d->aging_days
                    : (int) now()->diffInDays($d->created_at);
    ...
    public function escalate(Request $request, string $id)
    {
        ...
        $levels  = ['branch_officer', 'branch_manager', 'credit_committee', 'executive'];
        $current = array_search($disbursement->approval_level, $levels);
        $next    = $levels[min(($current !== false ? $current : 0) + 1, count($levels) - 1)];
    ...
    public function approve(Request $request, string $id)
    {
        ...
        // Super-approvers (system_admin, executive) can approve any amount
        $userRole = $user->role ?? 'branch_officer';
        $isSuperApprover = in_array($userRole, DisbursementService::superApproverRoles());

        if (!$isSuperApprover) {
            // Check authority level
            $matrix   = DisbursementService::authorityMatrix();
            $required = DisbursementService::requiredApprovalLevel($amount, $matrix);
    ...
    public function batch(Request $request)
    {
        ...
        // Response wrapped in 'data' key as expected by test
        return response()->json([
            'success' => true,
            'data'    => [
                'batch_id' => $batchRef,
    ...
    public function esignQueue(Request $request)
    {
        ...
        // Stats including 'expired' key as required by test
        $stats = [
            ...
            'expired' => Disbursement::where('esign_status', 'expired')
                ->orWhere(function ($q) {
                    $q->where('esign_status', 'pending')
                      ->where('esign_deadline', '<', now());
                })->count(),
    ...
    public function sendReminder(Request $request, string $id)
    {
        ...
        // Response with 'data' key containing id, reminder_sent_at, channel as expected by test
        return response()->json([
            'success' => true,
    ...
    public function offerLetterData(Request $request, string $id)
    {
        ...
        // Generate simple repayment schedule (first 3 months)
        $schedule = [];
