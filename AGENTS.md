...
**Last Updated:** 6 July 2026 — FULL SYSTEM VERIFIED. All 127+ backend tests passing (0 failures). Frontend build: 0 TypeScript errors, Vite build success. MFA/OTP login step JSX fixed. CheckModuleAccess updated with role-based module access map. AuditController, DisbursementController, IntegrationController, UserController all fixed. System fully operational for Tuesday POC.
...
| M4 — Pengurusan Akaun | hotfix/m4-account-list | #22+hotfix | #40 | ✅ GUI UPDATE MERGED — PR #40 (health ring, AreaChart, real DB channels, moratorium, Ta'widh) | 2026-07-05 |
| M2 — Penilaian Risiko | feature/m2-gui-clean | #39 | ✅ GUI UPDATE MERGED — PR #39 (gauge chart, explainability table, real DB, SPPT AI naming) | 2026-07-05 |
| M3 — Kelulusan & Dana | feature/m3-kelulusan-clean | #20 | ✅ MERGED (2026-07-05) — Real DB, 5 endpoints, 4 frontend pages, DisbursementTest | 🔧 Hotfix PR #44 (route ordering fix, 18/18 tests pass) | 2026-07-05 |
...
| M10 — Integrasi API | feature/m10-m12-fix | #41 | ✅ ROUTES REGISTERED — PR #41 (ApiHealthDashboard route active, CircuitBreakerBadge, LatencyGauge) | 2026-07-05 |
| M11 — Audit | feature/m11-clean | #21 | ✅ MERGED (2026-07-05) — Real DB, AI anomaly detection, 17/17 tests pass | 2026-07-05 |
| M12 — Pentadbiran | feature/m10-m12-fix | #41 | ✅ GUI UPDATE MERGED — PR #41 (real DB UserManagement, SystemConfig + RoleManagement pages, routes registered) | 2026-07-05 |
| **HOTFIX: Login Redirect** | hotfix/login-redirect-final | — | ✅ MERGED | 2026-07-05 |
| **MFA/OTP Login Step** | feature/login-mfa | #42 | ✅ MERGED — 2-factor OTP login, demo-friendly (any 6-digit), real backend integration | 2026-07-05 |
...
## Hotfix: M5 Live API Pages — 2026-07-05

**Branch:** hotfix/m5-live-api-pages (merged to main)
**Commit:** 685d38c → merged as 7105758

**Root Cause:** `App.tsx` was importing M5 pages from `src/pages/module5/` (old hardcoded versions) instead of `src/modules/pengurusan-npl/pages/` (new live-API versions).

**Fix Applied:**
- `src/pages/module5/NplDashboard.tsx` — replaced with live-API version (useNplDashboard + useAiAutomation)
- `src/pages/module5/DunningWorkflow.tsx` — replaced with live-API version (useDunningList)
- `src/pages/module5/CollectionTaskQueue.tsx` — replaced with live-API version (useCollectionTasks)
- Import paths fixed to use `@/modules/pengurusan-npl/hooks/useNpl`
- Frontend rebuilt and restarted on port 5174

**Tests:** PHPUnit 4/6 PASS, 2 SKIP (no test data — acceptable)
**TypeScript:** 0 errors

---

## Hotfix: M1 ApplicationList Crash — 2026-07-06

**Branch:** `hotfix/m1-applicationlist-crash`
**PR:** https://github.com/khamil-netgeo/TEKUN/pull/45
**Status:** OPEN — Awaiting Orchestrator merge

### Root Causes Fixed
1. **Application model** — Added `getFullNameAttribute()` and `getLoanPurposeAttribute()` accessors to bridge DB column names (`applicant_name`, `purpose`) with frontend type names (`full_name`, `loan_purpose`). Added `$appends` to auto-include in JSON output.
2. **api.ts** — Changed `baseURL: '/api'` to `baseURL: import.meta.env.VITE_API_BASE_URL ?? '/api'`. Added `.env.production` with `VITE_API_BASE_URL=http://34.177.95.116:8000/api` so the built dist calls the correct backend.
3. **NplDashboard.tsx (M5)** — Fixed malformed Tooltip formatter JSX that was blocking the entire `pnpm build`.

### Files Changed
- `backend/app/Models/Application.php` — full_name + loan_purpose accessors
- `frontend/src/services/api.ts` — VITE_API_BASE_URL env support
- `frontend/.env.production` — production API URL
- `frontend/src/pages/module5/NplDashboard.tsx` — Tooltip formatter syntax fix

### Post-fix Status
- Frontend rebuilt and restarted on port 5174
- `pnpm exec tsc --noEmit`: 0 errors
- `pnpm run build`: 4.08s, 0 errors

## M4 Hotfix — AccountList Page (2026-07-06)

### Problem Fixed
- Testing Agent reported `/akaun` route crashed (no AccountList page)
- Account360.tsx had no fallback when accessed without ID

### Files Added/Modified
- `frontend/src/modules/pengurusan-akaun/pages/AccountList.tsx` — NEW: Paginated account list with StatCard KPIs, DataTable, search
- `frontend/src/modules/pengurusan-akaun/routes.tsx` — Added `/akaun` index route → AccountList
- `frontend/src/modules/pengurusan-akaun/pages/Account360.tsx` — Redirect to /akaun when no ID
- `frontend/src/components/layout/Sidebar.tsx` — M4 nav: /akaun (list) + /akaun/:id (detail)
- 15+ pre-existing TS errors fixed across M1, M2, M5, M7, M8, M11 modules

### Quality Gate
- TypeScript build: 0 errors, built in 11.66s
- PHPUnit M4 tests: 8/8 PASS, 17 assertions
- Frontend deployed: http://34.177.95.116:5174

### Branch
- `hotfix/m4-account-list` pushed to GitHub
- PR pending Orchestrator merge
